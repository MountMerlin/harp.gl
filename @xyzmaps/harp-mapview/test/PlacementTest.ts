...
import { Env } from "@xyzmaps/harp-datasource-protocol";
import { getTestResourceUrl } from "@xyzmaps/harp-test-utils";
import {
    FontCatalog,
    HorizontalAlignment,
    HorizontalPlacement,
...
import { getAppBaseUrl } from "@xyzmaps/harp-utils";
...
async function createTextCanvas(): Promise<TextCanvas> {
    let fontDir = getTestResourceUrl("@xyzmaps/harp-fontcatalog", "");
    if (inNodeContext) {
        fontDir = path.dirname(require.resolve("@xyzmaps/harp-fontcatalog/package.json"));
    }
