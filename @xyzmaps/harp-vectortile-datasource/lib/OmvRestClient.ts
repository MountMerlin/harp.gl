...
import { TileKey, TilingScheme } from "@xyzmaps/harp-geoutils";
import { DataProvider } from "@xyzmaps/harp-mapview-decoder";
import { ITransferManager, TransferManager } from "@xyzmaps/harp-transfer-manager";
...
export interface OmvRestClientParameters {
     ...
     *  - `{z}` - zoom level of tile, @see {@link @xyzmaps/harp-geoutils#TileKey.level}
     *  - `{x}` - horizontal coordinate of tile (column number),
     *            see {@link @xyzmaps/harp-geoutils#TileKey.column}
     *  - `{y}` - vertical coordinate of Tile (row number),
     *            see {@link @xyzmaps/harp-geoutils#TileKey.row}
     *
